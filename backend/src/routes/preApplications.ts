...
const PreAppSchema = z.object({
  ...
  dob: z
    .string()
    .refine((d) => differenceInYears(new Date(), new Date(d)) >= 18),
...
// Get all pre-apps (filter by status optional)
...
// Reject
router.post('/:id/reject', requireGrade(0), async (req: AuthRequest, res) => {
  const id = req.params.id;
  const preRepo = AppDataSource.getRepository(PreApplication);
  const pre = await preRepo.findOneBy({ id });
  if (!pre) return res.status(404).json({ error: 'Not found' });
...
router.put('/:id', requireGrade(0), async (req: AuthRequest, res) => {
  const id = req.params.id;
  const parse = PreAppSchema.safeParse(req.body);
  if (!parse.success)
    return res.status(400).json({ error: parse.error.errors });
  ...
  await createLog('PreApplication', id, 'Edit', req.user!.id, req.user!.grade, {
    before,
    after: pre,
  });
